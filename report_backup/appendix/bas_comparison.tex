...
\label{app:mcmc_logit}\label{app:mcmc_horseshoe}

% ---- mutate din corp pentru a respecta limita de pagini ----
\begin{table}[htbp]
  \centering
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{img/horseshoe_model/trace_beta_1.png}
  \caption{Representative trace plot for a subset of the regression coefficients
           $\beta_j$ under the horseshoe prior (three chains).}
  \label{fig:hs_trace_beta}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{img/horseshoe_model/trace_lambda_1.png}
  \caption{Representative trace plot for the local scales $\lambda_j$ and the
           global scale $\tau$ (three chains). These scale parameters mix more
           slowly than the coefficients.}
...
model-space search and from the horseshoe's continuous shrinkage - two
structurally different estimation strategies (discrete model averaging
vs.\ a single shrinkage prior) landing on the same handful of variables.
The one disagreement, \texttt{Class:\ Business} (PIP $0.993$,
$\kappa = 0.186$), is a genuine borderline case in both methods (BAS just
below its $0.5$ cutoff, the horseshoe's credible interval for it crossing
zero in Section~03) - not a real contradiction between them.
