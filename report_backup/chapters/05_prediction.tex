...
\paragraph{Training-set size.} At $n \approx 1200$ the data are modest for
twenty-two predictors, which is why several effects sit close to the decision
boundary. The refit on a subsample twelve times larger moves the coefficients
we report by at most $0.023$, so the conclusions are not fragile; but the two
predictors whose signal/noise verdict changes with the global scale would
likely be settled by more data rather than by a better prior.
