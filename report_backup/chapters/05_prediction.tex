...
\subsection{Findings}
Three results stand out, and survive every specification:
\begin{enumerate}[nosep,leftmargin=*]
  \item \textbf{Customer loyalty} is the strongest effect by a wide margin and
        the least shrunk coefficient at every global scale tested
        ($\kappa_j = \mathbf{0.07}$).
  \item \textbf{Inflight entertainment} and \textbf{Business} class follow,
        essentially tied ($\kappa_j = \mathbf{0.17}$ and $\mathbf{0.18}$).
        Entertainment is the service item the prior preserves most clearly.
  \item \textbf{Inflight wifi service} is heavily shrunk
        ($\kappa_j = \mathbf{0.73}$) despite correlating at $\mathbf{0.67}$
        with \textbf{Online boarding} inside the same booking cluster. That
...
$\mathbf{82.8\%}$ correctly at the base-rate cutoff, and reproduces the
observed satisfaction rate in every subgroup.
