...
prefer fitting a Bayesian logistic regression on the \emph{full} set of covariates under a \textbf{horseshoe} shrinkage prior \cite{bl_book}, which is designed to pull the coefficients of
...
plus \texttt{age}, \texttt{flight\_distance}, the two class dummies
(\textit{Eco Plus}, \textit{Business}), $\log(1+\texttt{dep\_delay})$,
\textit{Personal Travel} and \textit{disloyal Customer}.
...
\emph{local} scale $\lambda_j$, and all coefficients share a single
...
prior, $\beta_0 \sim \mathcal{N}(0, 100)$, and is
...
budget a longer sampling run and thinning from the outset; the settings are
reported in Table \ref{tab:hs_mcmc_settings}.
...
\begin{table}[htbp]
  ...
  \begin{minipage}[t]{0.66\textwidth}
    ...
    plots. The coefficients mix well: every one has a scale-reduction factor of $1.00$
    and an effective sample size in the thousands. The local scales $\lambda_j$ mix
    more slowly and show higher autocorrelation, as expected for a horseshoe, and
    three of them sit at or above the conventional $1.1$ threshold; the global scale
    $\tau$, by contrast, is among the better-behaved parameters in that block.
    Figures \ref{fig:hs_trace_beta} and \ref{fig:hs_trace_lambda} show a
    representative trace plot for each block. The complete diagnostic material --
    all trace and autocorrelation plots for both blocks, together with the full
    Gelman--Rubin and effective-sample-size tables -- is collected in
    Appendix~\ref{app:horseshoe_convergence}, where the three imperfectly converged
    scales are identified and their consequences discussed.
  \end{minipage}
\end{table}

\begin{figure}[!ht]
  \centering
  \begin{minipage}[c]{0.343\linewidth}
    \centering
  ...
  \end{minipage}
  \hfill
  \begin{minipage}[c]{0.637\linewidth}
    \centering
    \begin{subfigure}[b]{\linewidth}
      \centering
      \includegraphics[width=\linewidth]{img/horseshoe_model/post1.png}
      \caption{Marginal posterior densities, first group; the remaining three
               are in Appendix~\ref{app:hs_shrinkage}.}
      \label{fig:hs_posterior}
    \end{subfigure}
  \end{minipage}
  \caption{Representative trace plots under the horseshoe prior (three chains,
           left) and the resulting marginal posteriors (right).}
\end{figure}
...
ranking is made explicit by the shrinkage weights $\kappa_j$ of
Eq.~\eqref{eq:kappa}, plotted in Appendix~\ref{app:hs_shrinkage}: predictors on
the ``signal'' side ($\kappa_j$ small) are the ones the model retains, while
those pushed to $\kappa_j \to 1$ are treated as redundant.
Figure \ref{fig:hs_forest}, in Appendix~\ref{app:hs_shrinkage}, reports the same information on the
...
items in the model ($\kappa_j = 0.74$ and $0.73$). This is exactly the mechanism the horseshoe was chosen for: when
...
of the strongest effects.} In Section~\ref{sec:logit}, \texttt{Business} class alone carried a
large, confident odds ratio (about 4). Here, with all 14 service ratings included,
its coefficient drops to approximately 0.9 on the log-odds scale ($\kappa = 0.183$),
...
in magnitude than the unconditional estimate of Section~\ref{sec:logit}, but not remotely
``explained away.'' The class effect is real but largely indirect:
Business passengers are more satisfied largely \emph{because} they enjoy better seat
comfort, entertainment and service, so in Section~\ref{sec:logit} the \texttt{Class} dummy was
...
although only just ($[-1.03,\,-0.002]$). The two findings are not
