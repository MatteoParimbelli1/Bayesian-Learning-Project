...
\newline
\hfill\newline
As stated before, we assume a Bernoulli likelihood with a logit link:
\begin{equation}
    y_i \mid p_i \sim \text{Bernoulli}(p_i),
    \qquad
    \text{logit}(p_i) = \beta_0 + x_i^\top \beta
        + \beta_{\text{mid}}\cdot\texttt{midflight\_delay}_i .
...
This treats the (standardised) mid-flight delay as a stochastic node in JAGS rather than
...
Applicable Information Criterion (WAIC) \cite{watanabe2010waic}, computed from the pointwise
...
Brier score \cite{brier1950} (0.187 against 0.190), and AUC (0.786 against 0.781). Since the
...
Convergence was satisfactory for all parameters. The Gelman--Rubin \cite{gelman1992rubin} potential
...
personal-travel coefficient) to roughly $1350$, which is adequate for
...
\begin{tabular}{lrrrrrr}
    ...
    Mid-flight delay (signed-log)        & $-0.041$ & $-0.187$ & $0.111$  & $0.960$ & $0.830$ & $1.118$ \\
...
(OR $\approx 0.16$, $[0.11, 0.24]$, i.e.\ $\bar\beta = -1.83$). Cabin class
comes next: travelling in \textit{Business} rather than \textit{Eco}
multiplies the odds by roughly $4.3$ ($95\%$ CI $[3.07, 6.08]$,
$\bar\beta = +1.45$), even after adjusting for all the other covariates.
\textit{Seat comfort} follows (OR $\approx 1.88$ per standard deviation,
$[1.63, 2.16]$): more comfortable seating is strongly associated with
satisfaction. A longer departure delay reduces the odds too
(OR $\approx 0.80$ per unit of $\log(1+\text{delay})$, $[0.69, 0.93]$).
