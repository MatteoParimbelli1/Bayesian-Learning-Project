...
%---------------- Float placement -----------------------------
% Defaults let LaTeX bail out to a dedicated float page as soon as the
% deferred floats fill half a page (\floatpagefraction=0.5). Ch.1 ends with a
% ~9.7cm figure and a ~5.5cm table and no trailing text, which trips exactly
% that rule and strands both on a page of their own. Raising the threshold
% above their combined height makes LaTeX set them at the top of a text page.
...
\input{chapters/00_prob_desc_data_expl}
\input{chapters/01_model}
\input{chapters/02_logit_model}
\input{chapters/03_horseshoe_model}
\input{chapters/04_sensitivity}
\input{chapters/05_prediction}

\begin{appendices}
\include{appendix/data_aug}
\include{appendix/logit_convergence}
\include{appendix/horseshoe_convergence}
\include{appendix/bas_comparison}
\end{appendices}
