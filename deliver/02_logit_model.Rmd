...
We start by fitting the model on the suggested covariates (or their transformation):`age`, `seat_comfort`, `flight_distace`, `class`, `log_dep_delay` and `log_mid_delay`.
...
Before actually evaluating the model, we focus on a small question: is it better to encode the `class` as dummy variable or as ordered one?

```{r fit-encoding, cache=TRUE, cache.extra=list(n_chains, n_adapt, burn_in, iter, thinning)}
set.seed(123)
